...
region_lambda_surv<-function (lambda1 =18,lambda2=0,iteration = 10, Y, G,X,center,w,K=K,delta){
  ...
  for (i in 1:length(num.vector)) {
    mod1 <- ogClust_Surv(x=X,G=t(G),y=Y,y.ind = delta,c_center=center,lambda=lambda.vector[i],v_int=NULL,pi_int=NULL,K=K,max_iter=200,w_outcome=w,w_G=1-w,z_int=NULL)
  ...
  while (iter <= iteration) {
    ...
    mod_new<-ogClust_Surv(x=X,G=t(G),y=Y,y.ind = delta,c_center=center,lambda=lambda,v_int=NULL,pi_int=NULL,K=K,max_iter=200,w_outcome=w,w_G=1-w,z_int=NULL)
